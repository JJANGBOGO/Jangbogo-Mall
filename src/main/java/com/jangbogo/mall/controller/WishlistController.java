...
public class WishlistController {
    ...
    public String list(Model m, HttpServletRequest request, HttpSession session) {  // 하트모양(위시리스트) 누₩르거나, 마이페이지에서 위시리스트 클릭 시
...
//        wishlistService.ge
        ...
        try {
            ...
            List<ProductDto> list2 = wishlistService.getlist(idx); // 회원번호를 알려주면 그 회원의 위시리스트를 보여준다(하드코딩)
            int totalCnt = list2.size(); // 이 회원의 위시리스트 총 상품 개수
...
//            List<ProductDto> list2 = wishlistService.getProduct(prods); // 상품번호들 넣어주면 그 상품의 정보를 받아온다
        ...
        return "wishlist"; // 로그인을 한 상태이면, 위시리스트 조회 화면으로 이동
    }
    ...
    public String delete(Integer prod_idx, HttpServletRequest request) { // 위시리스트 삭제
        ...
        // 세션에서 이메일 가저온뒤에 넣어주면 그 이메일을 가진 회원번호 알아낸다
        try {
            int cnt = wishlistService.delete(prod_idx, 1);  // 상품번호와 회원번호를 넣어 위시리스트에서 삭제한다
            if (cnt != 1) {
    ...
    public String insertCart(Integer prod_idx, Integer prod_cnt, Model m, RedirectAttributes ratt, HttpServletRequest request) {
        ...
        try {
            ...
            int CartCnt = wishlistService.checkCart(prod_idx, 1); // 상품번호, 회원번호(하드코딩)
            if (CartCnt != 1) { // 장바구니에 이 상품이 없으면
                int cnt = wishlistService.insertCart(prod_idx, 1, prod_cnt); // 상품번호,회원번호(하드코딩),개수(하드코딩)를 넣어주면 장바구니에 담긴다
                ratt.addFlashAttribute("msg", "add");
            } else {
                int cnt = wishlistService.updateCartCnt(prod_idx, 1, prod_cnt);
